...
CARPE.Slider.prototype = {
    ...
    move: function (evnt) {
        ...
        var result = true,
            v = this.vertical;
        if (this.sliding) {
            this.position = v * (this.startOffsetY + evnt.screenY) +
                (!v) * (this.startOffsetX + evnt.screenX);
            this.update();
            result = false;
        }
        return result;
        ...
        if (targets && targets.length) {
            ...
            for (i = 0; i < len; i += 1) {
                ...
                if (t.setValue && CARPE.isFunction(t.setValue)) {
                    t.setValue(val);
                } else if (Object.prototype.hasOwnProperty.call(t,
                        'value') || (t.constructor.toString().
